namespace System.Throw
{
    ...
    public class ThrowIfGreaterThanOrEqualTest<T>
    {
        private static void ThrowIfGreaterThanOrEqual(object value, object other)
        {
            if (value is not null && other is not null) {
                if (typeof(T) == typeof(int)) {
                    ...
                    ThrowHelper.ThrowIfGreaterThanOrEqual(arg, argOther);
                    return;
        ...
        {
            Assert.That(() => {
                ThrowIfGreaterThanOrEqual(0, 0);
            }, Throws.TypeOf<ArgumentOutOfRangeException>().With.Property("ParamName").EqualTo("arg"));

            Assert.That(() => {
                ThrowIfGreaterThanOrEqual(10, 10);
            }, Throws.TypeOf<ArgumentOutOfRangeException>().With.Property("ParamName").EqualTo("arg"));

            Assert.That(() => {
                ThrowIfGreaterThanOrEqual(10, 0);
            }, Throws.TypeOf<ArgumentOutOfRangeException>().With.Property("ParamName").EqualTo("arg"));

            ThrowIfGreaterThanOrEqual(0, 10);
            ...
            if (typeof(T) == typeof(EventLine)) {
                ...
                Assert.That(() => {
                    ThrowHelper.ThrowIfGreaterThanOrEqual(argn, null);
                }, Throws.TypeOf<ArgumentOutOfRangeException>().With.Property("ParamName").EqualTo("argn"));

                ThrowHelper.ThrowIfGreaterThanOrEqual(argn, arg1);
            }
        }
    }
}
