namespace System.Throw
{
    ...
    public class ThrowIfGreaterThanTest<T>
    {
        private static void ThrowIfGreaterThan(object value, object other)
        {
            if (value is not null && other is not null) {
                if (typeof(T) == typeof(int)) {
                    ...
                    ThrowHelper.ThrowIfGreaterThan(arg, argOther);
                    return;
        ...
        {
            ThrowIfGreaterThan(0, 0);
            ThrowIfGreaterThan(10, 10);

            Assert.That(() => {
                ThrowIfGreaterThan(10, 0);
            }, Throws.TypeOf<ArgumentOutOfRangeException>().With.Property("ParamName").EqualTo("arg"));

            ThrowIfGreaterThan(0, 10);

            if (typeof(T) == typeof(EventLine)) {
                EventLine arg1 = new(50, "Foo");
                EventLine argn = null;

                ThrowHelper.ThrowIfGreaterThan(argn, null);
                ThrowHelper.ThrowIfGreaterThan(argn, arg1);
            }
        }
    }
}
